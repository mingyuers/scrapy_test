...
class weiboLogin:
    ...
    def getServerData(self):
        ...
        print data
        # p = re.compile('[Math Processing Error]')
        data = str(data)
        data = data.replace("sinaSSOController.preloginCallBack(","").replace(')','')
        print data
        try:
    ...
    def login(self, username, psw):
        ...
        p = re.compile('location\.replace[Math Processing Error]')
        try:
            # login_url = p.search(text).group(1)
            soup = BeautifulSoup(text,'lxml')
            print soup.find_all(attrs={'url'})
            login_url = soup.script.string.replace(r'location.replace("','').replace(r'")','')
            ...
            # 访问主页，把主页写入到文件中
        url = 'http://weibo.com/u/1933727622/home?wvr=5&lf=reg'
...
wblogin = weiboLogin()
username = '970709406@qq.com'
password = '199358fgmasd'
wblogin.login(username,password)
# wblogin.getServerData()